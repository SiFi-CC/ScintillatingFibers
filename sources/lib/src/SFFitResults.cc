                                                     ...
                                                     fFunction(fun)
{
    ...
    if (!stat) { throw "##### Exception in SFFitResults constructor!"; }
}
//------------------------------------------------------------------
/// Default destructor.
...
bool SFFitResults::SetFromFunction(TF1* fun)
{
    ...
    if (fFunction == nullptr) fFunction = fun;
...
std::vector<TF1*> SFFitResults::GetCompFunctions(void)
{
    ...
    if (fComponents == 1) { functions.push_back(funDecay); }
    else if (fComponents == 2)
    {
...
void SFFitResults::Print(void)
{
    ...
    {
        std::cout << "Fast decay time: " << fFastDecTime << " +/- " << fFastDecTimeErr << " ns"
                  << std::endl;
        std::cout << "Slow decay time: " << fSlowDecTime << " +/- " << fSlowDecTimeErr << " ns"
                  << std::endl;
