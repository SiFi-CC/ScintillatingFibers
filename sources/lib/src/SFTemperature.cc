...
bool SFTemperature::LoadFromDB(TString sensorID, TString name){
  ...
  if(name=="none")
    query = Form("SELECT TIME, TEMPERATURE FROM TEMPERATURES WHERE SERIES_ID = %i AND SENSOR_ID = '%s'", fSeriesNo, sensorID.Data());
  else
    query = query = Form("SELECT TIME, TEMPERATURE FROM TEMPERATURES WHERE SERIES_ID = %i AND SENSOR_ID = '%s' AND MEASUREMENT_NAME = '%s'", fSeriesNo, sensorID.Data(), name.Data());
...
//------------------------------------------------------------------
TemperatureResults SFTemperature::CalcAverageTempMeasure(TString sensor, TString name){
...
bool SFTemperature::CalcAverageTempSeries(TString sensor){
  ...
  TemperatureResults temp;
  temp.fTemp = mean;
  temp.fTempErr = stDev;
  
  fAvTemp.insert(std::pair<TString, TemperatureResults>(sensor, temp));
...
//------------------------------------------------------------------
TemperatureResults SFTemperature::GetAverageTempSeries(TString sensor){
    
  std::map <TString, TemperatureResults>::iterator itr; 
  ...
  TemperatureResults temp = itr->second;

  if(temp.fTemp==-1 || temp.fTempErr==-1){
      std::cerr << "##### Error in SFTemperature::GetAverageTempSeries()!" << std::endl;
